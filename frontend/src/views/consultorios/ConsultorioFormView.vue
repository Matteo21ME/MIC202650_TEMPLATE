...
const form = ref({ piso: '', estado: 'DISPONIBLE', direccion: '' })
const errores = ref({}); const enviando = ref(false); const cargando = ref(false)

async function cargar() {
  cargando.value = true
  try { const { data } = await consultorioService.getById(route.params.id); Object.assign(form.value, data) }
  catch { alert('No se pudo cargar.'); router.push('/consultorios') }
...
async function enviar() {
  ...
  try {
    esEdicion.value
      ? await consultorioService.update(route.params.id, form.value)
      : await consultorioService.create(form.value)
  ...
  <div class="page">
    <div class="form-card">
      <div class="form-header">
        <h1>{{ esEdicion ? 'Editar Consultorio' : 'Nuevo Consultorio' }}</h1>
        <RouterLink to="/consultorios" class="btn-back">← Volver</RouterLink>
      </div>
      <div v-if="cargando" class="estado">Cargando...</div>
      <form v-else @submit.prevent="enviar" novalidate>
        <div class="form-grid">
          <FormField label="Piso" :error="errores.piso">
            <input v-model="form.piso" type="text" maxlength="50" required :class="{ invalid: errores.piso }" />
          </FormField>
          ...
          <FormField label="Dirección" :error="errores.direccion" :full="true">
            <input v-model="form.direccion" type="text" maxlength="100" required :class="{ invalid: errores.direccion }" />
          </FormField>
        </div>
        <div class="form-footer">
          <p class="hint">Verifica los datos antes de guardar.</p>
          <button type="submit" class="btn-submit" :disabled="enviando">
            {{ enviando ? 'Guardando...' : (esEdicion ? 'Actualizar' : 'Crear Consultorio') }}
          </button>
        </div>
      </form>
    </div>
