...
class StoreCitaMedicaRequest extends FormRequest
{
    ...
    {
        return [
            'medico_id'      => ['required', 'integer', 'exists:microbusiness.medicos,id'],
            'paciente_id'    => ['required', 'integer', 'exists:microbusiness.pacientes,id'],
            'consultorio_id' => ['required', 'integer', 'exists:microbusiness.consultorios,id'],
            'Dia_Semana'     => ['required', 'string', 'in:Lunes,Martes,Miércoles,Jueves,Viernes'],
            'hora'           => ['required', 'regex:/^([01]\\d|2[0-3]):[0-5]\\d(:[0-5]\\d)?$/'],
            'motivo'         => ['required', 'string', 'max:255'],
            'estado'         => ['required', 'string', 'in:PROGRAMADA,ATENDIDA,CANCELADA'],
        ];
    }
